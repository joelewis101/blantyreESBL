...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_exposures.svg",exp_plot,width = 8, height = 5)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_exposures.pdf",exp_plot,width = 8, height = 5)
...
```{r sample_timing, fig.cap = "SUPPLEMENTARY FIGURE X: Timing of sample collection showing long tails around sample collection time.", fig.height = 4, fig.width= 6 }
btESBL_stoolESBL %>%
  filter(visit != 0) %>% 
...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_sample_collection_dates.svg",samp_col_dates, width = 6, height = 4)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_sample_collection_dates.pdf",samp_col_dates, width = 6, height = 4)
...
btESBL_stoolorgs %>% 
  mutate(
    organism = 
      case_when(organism == "pantoea sp" ~ "Panotea species",
                organism == "Gram negative bacilli" ~
                  "Gram negative bacilli*",
                TRUE ~ organism)) %>% 
  ggplot(aes(fct_rev(fct_infreq(organism)))) +
  geom_bar() +
  coord_flip() +
  labs(y = "n",
       x = "")+
...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_stool_spec.svg",stool_spec, width = 6, height = 4)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_stool_spec.pdf",stool_spec, width = 6, height = 4)
...
btESBL_model2simulations %>%
    ...
    mutate(abx_stop = paste0(as.character(abx_stop), 
                             " days \nantimicrobials")) %>%
    ggplot(aes(
        time,
        median,
...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F1_markov_panel.svg",markov_panel_plot,width = 10, height = 7)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F1_markov_panel.pdf",markov_panel_plot,width = 10, height = 7)
```
...
ggsave( "~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_model_comp_plot.svg", model_comp_plot, width = 11, height = 8)

ggsave( "~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_model_comp_plot.pdf", model_comp_plot, width = 11, height = 8)
...
```{r plasm-amr-gene-plot, fig.width = 10, fig.height = 12, fig.cap = "FIGURE 2: Species and lineage association of mobile genetic elements. A-B: Jaccard-distance hierarchical clustered heatmap of presence-absence matrix of antimicrobial resistance genes (A) and plasmid replicons (B). Yellow= present, purple = absent. Each row is a sample, and each column an AMR gene (A) or plasmid replicon (B), showing that the samples tend to cluster by species. C: Distribution of ESBL contig clusters by species showing that most contog-clusters are species associated or restricted. D-E: Contig clusters mapped back to E. coli phylogeny (D) or K. pneumoniae KpI phylogeny showing that most contig clusters are lineage-associated."  }

# plasmid replicons clustered heatmap --------------------------
...
btESBL_plasmidreplicons %>%
  ...
  as.data.frame() -> plasmids_onehot

rownames(plasmids_onehot) <- plasmids_onehot$sample

as.ggplot(
  pheatmap(
    plasmids_onehot[-c(1, 2)],
    annotation_row = plasmids_onehot["Species"],
    ...
    treeheight_col = 0,
    silent = TRUE,
    annotation_colors = list(Species = 
                               c("E. coli" = viridis_pal()(4)[2],
                                 "K. pneumoniae" = viridis_pal()(4)[3])
    )
...
btESBL_amrgenes %>%
  ...
  relocate(species, before = everything()) %>%
  rename(Species = species) %>% 
  as.data.frame() -> amr_onehot

rownames(amr_onehot) <- amr_onehot$sample

as.ggplot(
  pheatmap(
    amr_onehot[-c(1, 2)],
    annotation_row = amr_onehot["Species"],
    annotation_names_row = FALSE,
    show_rownames = FALSE,
    show_colnames = FALSE,
    legend = FALSE,
...
#amr_heatmap + plasm_heatmap + plot_layout(widths = c(1,1.4), tag_level = "A")
...
#(
#  (amr_heatmap + plasm_heatmap + plot_layout(widths = c(1,1.6))) /
#    (
#      (contigs_species_distnplot | 
...
species_lineage_mge_association
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F2_spec_lin_mgs_assoc.svg",species_lineage_mge_association, width = 10, height = 9)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F2_spec_lin_mgs_assoc.pdf",species_lineage_mge_association, width = 10, height = 9)

#ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F2_spec_lin_mgs_assoc.pdf",species_lineage_mge_association, width = 10, height = 12)

```

```{r sup-amr-heatmap, fig.cap = "SUPPLEMENTARY FIGURE: Enlarged AMR gene (A) and plasmid replicon (B) jaccard-distance hierarchical-clustered heatmaps. Each row is a sample, each column an AMR gene or plasmid replicon. Yellow = present, purple = absent.", fig.width = 18, fig.height = 10}
as.ggplot(
  pheatmap(
    amr_onehot[-c(1, 2)],
    annotation_row = amr_onehot["Species"],
...
amr_heatmap_sup / plasm_heatmap_sup +
  plot_annotation(tag_levels = "A") +
  plot_layout(heights = c(1, 1.5)) -> supp_spec_heatmap

supp_spec_heatmap

#ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_species_mge_heatmap.jpg", supp_spec_heatmap, width = 18, height = 10)

```

```{r contig-cluster-stats, fig.cap = "SUPPLEMENTARY FIGURE XXX: Size, representative cluster length, distribution of coverage and sequence identity of ESBL-contig clusters", fig.width=9, fig.height=7}
...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_esbl_contig_stat.svg", esbl_contig_stats, width = 9, height = 7)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_esbl_contig_stat.pdf", esbl_contig_stats, width = 9, height = 7)

```

### popPUNK clusters mapped to phylogeny

```{r popPUNK+map_to_phylogeny, fig.width= 12, fig.height=10, fig.cap = "SUPPLEMENTARY FIGURE XXX: popPUNK clusters mapped to core gene maximum-likelihood phylogeny"}
...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_popunk_maptotree.svg", pp.maptotree, height = 10, width = 12) 

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_popunk_maptotree.pdf",pp.maptotree, height = 10, width = 12) 

pp.maptotree

```

### Within-participant temporal correlation

```{r autocorrelation-plot, fig.height = 7, fig.width = 9, fig.cap = "Within participant correlation of organism, lineage, mobile genetic element (MGE) and clone. A-B show proportions of participants that have a detectible E. coli (A), K. pneumoniae (B) at t time t=0 and the same organism at a time t later. Dashed lines show the proportion of samples in the collection that would be expected to have the same organism by chance: the proportion of between-participant samples that have the same organism. C shows (for E. coli only) the proportion of participants who are colonised with E. coli at time t=0 and the proportion of those who have the same lineage (as defined by popPUNK cluster), clone (as defined by <6 SNPs), or mobile genetic element (defined by ESBl-contig cluster) detectible a at time t later. Dashed lined again show the between-participant proportion of samples that contain the same lineage, clone or MGE to show the chance of obtaining any result ny chance. Fitted lines are from a nonparametric LOESS fit with 95% confidence intervals. D shows odds ratios of any within-participant E. coli pair containing the same popPUNK cluster only, ESBL-contig cluster, or both, compared to between-participant pairs. It is more likely that both lineage (popPUNK cluster) and MGE (ESBL-cluster) are conserved together than either seperately, suggesting that the lineage-MGE combination is the conserved unit within this system." }
...
autocorrelation_plot
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F3_autocorrelation.svg", autocorrelation_plot, height = 9, width = 9)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F3_autocorrelation.pdf", autocorrelation_plot, height = 9, width = 9)


```

```{r kleb-popunk-mge-plot, fig.cap = "SUPPLEMENTARY FIGURE XXXX: For K. pneukmoniae the proportion of participants who are colonised with K. pneukoniae at time t=0 and the proportion of those who have the same lineage (as defined by popPUNK cluster), clone (as defined by <6 SNPs), or mobile genetic element (defined by ESBl-contig cluster) detectible a at time t later. Dashed lined again show the between-participant proportion of samples that contain the same lineage, clone or MGE to show the chance of obtaining any result ny chance. Fitted lines are from a nonparametric LOESS fit with 95% confidence intervals.", fig.width=6, fig.height=4, include = FALSE}

#k2

#ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_kleb-autocorrelation-plot.svg", k2, height = 4, width = 6)

#ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_kleb-autocorrelation-plot.pdf", k2, height = 4, width = 6)

```

### Hospital associated lineages and MGE

```{r hosp-assoc-lineages, fig.height = 8, fig.width = 9}

btESBL_popPUNK %>% 
  left_join(unique(btESBL_sequence_sample_metadata),
            by = c("Taxon" = "lane")) %>% 
  as.data.frame() -> clusts

rownames(clusts) <- clusts$Taxon
...
ggplot(df.manhattan.popPUNK, aes(clustr, log_pval, color = log_pval > 
  ...
  labs(y = "-log(p)", x = "popPUNK cluster") -> manhattanplot_popPUNK
 # 
...
ggplot(df.manhattan.esbl, aes(clstr_name, log_pval, color = log_pval > 
  ...
  theme(
    ...
    axis.text.x = element_blank()) +
      scale_color_manual(values = c("black", "red")) +
  labs(y = "-log(p)", x = "ESBL-contig cluster") -> manhattanplot_esbl
 # 
...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F4_hosp_assoc_lineages.svg", hosp_assoc_lineages, height = 8, width = 9)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F4_hosp_assoc_lineages.pdf", hosp_assoc_lineages, height = 8, width = 9)
  
```


```{r snp-clusts, fig.width = 13.2, fig.height = 6}
...
ggraph(gr.e, layout = "fr") + geom_edge_fan(aes(color = type), width =1) + 
...
(snp_network_plot.esco + 
  ...
  plot_annotation(tag_levels = "A") -> snp_networks
snp_networks

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F4snp_networks.svg", snp_networks, width = 13.2, height = 6)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/F4snp_networks.pdf", snp_networks, width = 13.2, height = 6)
...
```{r snp-cutoff-sens-analysis, fig.width = 9, fig.height = 4, fig.cap = "SUPPLEMENTARY FIGURE: Sensitivity analysis showing proportion of within-participant sample pairs that contain E. coli (A) or K. pneumoniae (B) that are within a given SNP distance."}
...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_sens_analysis_snpdist_autocorrelation.svg", sens_analysis_within_snpdist, width = 9, height = 4)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_sens_analysis_snpdist_autocorrelation.pdf", sens_analysis_within_snpdist, width = 9, height = 4)

```

```{r snp-network-sens-analysis, fig.width = 13.2, fig.height = 12, fig.cap = "SUPPLEMENTARY FIGURE: Sensitivity analysis of network plot of putative transmission clusters for E. coli, allowing the definition of a SNP-cluster to vary from 0 (A), to 3 (B), 7 (C), or 10 (D). Points are samples, coloured by place of isolation (in-hospital [black], community [white], or up to 120 days post-discharge [grey]). Red lines link samples that are within a single participant. Blue lines link samples that are differ by the given SNP threshold."}
...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_sens_analysis_snp_networks_esco.svg", esco_snp_clusters_sens_analysis_plot, width = 13.2, height = 12)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_sens_analysis_snp_networks_esco.pdf", esco_snp_clusters_sens_analysis_plot, width = 13.2, height = 12)

```

```{r snp-network-sens-analysis_kleb, fig.width = 13.2, fig.height = 12, fig.cap = "SUPPLEMENTARY FIGURE: Sensitivity analysis of network plot of putative transmission clusters for E. coli, allowing the definition of a SNP-cluster to vary from 0 (A), to 3 (B), 7 (C), or 10 (D). Points are samples, coloured by place of isolation (in-hospital [black], community [white], or up to 120 days post-discharge [grey]). Red lines link samples that are within a single participant. Blue lines link samples that are differ by the given SNP threshold."}

kleb_snp_clusters_sens_analysis_plot

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_sens_analysis_snp_networks_kleb.svg", kleb_snp_clusters_sens_analysis_plot, width = 13.2, height = 12)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_sens_analysis_snp_networks_kleb.pdf", kleb_snp_clusters_sens_analysis_plot, width = 13.2, height = 12)

```

```{r posterior-pairs-plot, fig.width= 12, fig.height=12, fig.cap = "Pairs plots of posterior of final model showing nonidentifiability of gain and loss parameters."}

mcmc_pairs(btESBL_model2posterior, pars = c("alphas[1]", "betas[1]", "alphas[2]", "betas[2]", "lambda", "mu", "gammas[1]")) -> pairsplot

pairsplot

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_pairsplot.svg", pairsplot, width = 12, height = 12)



ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_pairsplot.pdf", pairsplot, width = 12, height = 12)
...
ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_SNP_sens_ax_clusters.svg",sens.analysis.plot,width = 6.5, height = 5)

ggsave("~/Documents/PhD/Manuscripts/2021012_esbl_carriage/manuscript/tables_and_figs/SUPP_FIG_SNP_sens_ax_clusters.pdf",sens.analysis.plot,width = 6.5, height = 5)
