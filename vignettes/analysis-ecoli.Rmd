...
(
    (
      (
        (
          ggtree(btESBL_ecoli_globaltree_noASC, size = 0.2) %<+%
...
(
  ggtree(
    treeio::tree_subset(
      btESBL_ecoli_globaltree_noASC, 3565, levels_back = 0)
...
(
  ggtree(
    treeio::tree_subset(
      btESBL_ecoli_globaltree_noASC, 5652, levels_back = 0)
    ) %<+% select(df_all, lane, cluster_recode) %>%
    gheatmap(
      select(df_all, Country) %>%
      mutate(Country = if_else(
...
(
  ggtree(
    treeio::tree_subset(
      btESBL_ecoli_globaltree_noASC, 5404, levels_back = 0)
    ) %<+% select(df_all, lane, cluster_recode) %>%
    gheatmap(
    select(df_all, Country) %>%
      mutate(Country = if_else(
