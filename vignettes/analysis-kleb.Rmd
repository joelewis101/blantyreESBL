...
rownames(spec_hm) <- spec_hm$strain
  ...
  ggtree(treeio::tree_subset(btESBL_coregene_tree_kleb, 218, levels_back = 0)) %>% 
    gheatmap(
      select(mlst.onehot,-lane),
      width = 2, 
      color = NA, 
...
(
  ...
  geom_cladelabel(node = 523, label = "ST307", align = TRUE, offset = - 0.0013,
                  fontsize = 3) +
  geom_treescale(x = 0.0003, y = 330, offset = 2, width = 0.001,
                  fontsize = 3) -> malawi_tree_plot_final
