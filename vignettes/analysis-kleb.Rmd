---
title: "Reproducible analysis scripts"
...
Joseph M Lewis^1,2,3,4^, , Madalitso Mphasa^1^, Rachel Banda^1^,  Matthew Beale^4^, Jane Mallewa^5^, Eva Heinz^2^, Nicholas Thomson^4^, Nicholas A Feasey^1,2^
...
if (save_figs) {
  ggsave(
    filename = here("data-raw/figures/F1_stplot.svg"),
    plot = stplot,
    width = 8,
    height = 11
  )
...
if (save_figs) {
ggsave(filename = here("data-raw/figures/SUP_F_dassimtrees.svg"),
       plot = dassim_trees,
       width =14,
       height = 16
)
...
if (save_figs) {
  ggsave(
    filename = here("data-raw/figures/F2_amr_plot.svg"),
    plot = amrplot,
    width = 6,
    height = 10
  )
...
if (save_figs) {
ggsave(filename = here("data-raw/figures/SUP_F_amr_heatmap.svg"),
       plot = amr.heatmap,
       width =9,
       height = 8
)

ggsave(filename = here("data-raw/figures/SUP_F_amr_heatmap.pdf"),
       plot = amr.heatmap,
       width =9,
       height = 8
)
...
rownames(amr.ariba.maptotree) <- amr.ariba.maptotree$name
...
#ggtree(tree) %>% 
    gheatmap(
        select(amr.ariba.maptotree,-name),
        width = 5, 
        color = NA, 
        font.size = 3, 
        colnames_angle = 90, 
...
if (save_figs) {
  ggsave(
    filename = here("data-raw/figures/SUP_F_amr_tree.pdf"),
    plot = tree_amr_plt,
    width = 14,
    height = 12
  )
  ...
  ggsave(
    filename = here("data-raw/figures/SUP_F_amr_tree.svg"),
    plot = tree_amr_plt,
    width = 14,
    height = 12
  )
...
if (save_figs) {
  ggsave(
    filename = here("data-raw/figures/F3_globaltree.svg"),
...
ggtree(treeio::tree_subset(dassimKleb_trees.malawi, 372, levels_back = 0)) %>%
...
if (save_figs) {
ggsave(filename = here("data-raw/figures/F4_malawitree.svg"),
...
if (save_figs) {
  ggsave(
    filename = here("data-raw/figures/SUPP_FIG_virplot.svg"),
    plot = virplot,
    width = 5,
    height = 5
  )
...
if (save_figs) {
  ggsave(
    filename = here("data-raw/figures/F5_OK_type_distn.svg"),
  ...
  ggsave(
    filename = here("data-raw/figures/F5_OK_type_distn.pdf"),
    plot =  o_ktype_distn_plot,
    width = 10,
    height = 4
  )
